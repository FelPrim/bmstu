...
int main(){
	...
	double *y_ = malloc(node_count*sizeof(double));
	...
	for (int i = 0; i < node_count; ++i){
		for (int j = 0; j < node_count; ++j){
			...
			F[i*node_count +j] = delta - lambda * K(x[i], x[j]) * h;
		}
	}

//	PRINT_MATRIX(node_count, F);
	SLAE_solve(node_count, F, x, y_);
//	PRINT_MATRIX(node_count, F);
