...
$\begin{matrix}\vec{\omega_{e}}=0 
\\
 \vec{\varepsilon_{e}}=0 
\\
 \vec{a_{O'}}=0 \end{matrix}$
...
\\
M\vec{a}_{C}=\vec{R}^{(\text{в})}, \text{ где 
$M=\sum _{i}m_{i}$
 }
\end{gather}$$
...
\\
\vec{v}_{ч}=\vec{v}_{л}+\vec{v}_{r}, \text{ где 
...
\\
=\sum (\vec{r}_{C}+\vec{\rho}_{i})\times m_{i}(\vec{v}_{C}+\vec{v}_{i}^{(r)})=
\\
 =\sum \vec{r}_{C}\times m_{i}\vec{v}_{C}+\sum \vec{\rho}_{i}\times m_{i}\vec{v}_{C}+\sum \vec{r}_{C}\times m_{i}\vec{v}_{i}^{(r)}+\sum \vec{\rho}_{i}\times m_{i}\vec{v}_{i}^{(r)}=
\\
...
$\begin{matrix}i\in\{1,\ldots,n\}
\\
 j\in \{ 1,\ldots,m \}\end{matrix} \ \{ \vec{F}_{ij},\vec{\text{Ф}}_{i} \}\sim \{ \vec{0} \}$
