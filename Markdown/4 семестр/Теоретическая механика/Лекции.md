...
\end{gather}$$
Проводим эксперимент по каждой координате
$$\begin{gather}
...
\delta A(\vec{F}^{\text{д}})=-\mu  \dot{\vec{r}}\sum _{i}\frac{ \partial \vec{r} }{ \partial q_{i} } \delta q_{i} =\\
...
a-\text{обобщённый коэффициент энергии} \\
