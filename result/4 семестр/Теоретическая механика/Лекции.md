...
m\vec{a_{r}}=\Sigma F+\vec{Ф_{e}}+\vec{Ф_{K}} \end{gather}$$
...
\vec{F}=f(\vec{r})\vec{r} \end{gather}$$
...
\frac{d\vec{r}}{dt}\times \vec{V}+\vec{r}\times \frac{d\vec{V}}{dt}=\cancel{ \vec{V}\times \vec{V} }+\vec{r}\times \vec{a}=\vec{r}\times \frac{f(\vec{r})}{m}\vec{r}=0 \end{gather}$$
...
f(x,y,z)=0
\end{cases} \\ 
\text{Решается через множитель Лагранжа.} \end{gather}$$
...
\sum _{i}m_{i}\vec{a}_{i}=\vec{R}^{(\text{в})} \end{gather}$$
...
\vec{V}_{C}=\frac{\sum _{i}m_{i}\vec{V}_{i}}{\sum _{i}m_{i}}=\vec{const} \end{gather}$$
...
M_{C}V_{C}^x=const \\ 
\text{ Пусть  $V_{C}\vert_{t=0}=0$  }\Rightarrow  \\ 
v_{\text{цм}}^x=0 \\ 
Q^x=0\implies  \\ 
...
$$\begin{gather}
\vec{r}_{i}\times m_{i}\vec{a}_{i}=\sum _{j}\vec{r}_{i}\times \vec{F}_{iO}=\sum _{j}\vec{M}_{O}(\vec{F}_{ij}) \end{gather}$$
...
\frac{d\vec{K}^O}{dt}=\sum _{ij} \vec{M}^O(\vec{F}_{iO})=\sum \vec{M}_{O}\vec{F}^\text{внеш}+\sum \vec{M}_{O}\vec{F}^\text{внутр} \end{gather}$$
...
$$\begin{gather}
\vec{n}=\frac{\overrightarrow{\text{grad} f}}{\lvert \vec{\text{grad} f} \rvert }  \\ 
\overrightarrow{\text{grad} }f=\begin{pmatrix}
\frac{ \partial f }{ \partial x }   \\ 
\frac{ \partial f }{ \partial y }   \\ 
\frac{ \partial f }{ \partial z } 
...
$$\begin{gather}
\text{\vec{\text{grad} }} f\cdot \delta \vec{r}=\frac{ \partial f }{ \partial x } \delta x+\frac{ \partial f }{ \partial y } \delta y+\frac{ \partial f }{ \partial z } \delta z
\end{gather}$$
