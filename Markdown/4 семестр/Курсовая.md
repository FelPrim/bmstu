...
$$\begin{gather}
i^2\sin(2y)\text{ ch } (2x)+i\cos(2y)\text{ sh } (2x)+C \\
\text{ ch } 2z=\text{ ch } (2x+i2y)=\text{ ch } 2x\text{ sh } (i 2y)+\text{ ch } (i 2y)\text{ sh } (2x)= \\
\text{ sh } iz= \frac{e^{iz}-e^{-iz}}{2i}i=i\sin z \\ 
\text{ ch } iz=\frac{e^{iz}+e^{-iz}}{2}=\cos z \\
=i\text{ ch } 2x\sin2y +\cos 2y\text{ sh } 2x= \\
=
\end{gather}$$